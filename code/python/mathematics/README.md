...
The chapter develops examples before general statements. The editorial reference
...
From the repository root, create the optional rendering environment:
    ...
    micromamba run -n mathematics-animations python code/python/mathematics/render.py LinearMap Stability Taylor Jacobian KKT Sampling NormalArea Fourier
...
The eight H.264 movies are 1280×720 at 30 frames per second, with browser-friendly
pixel format and fast-start metadata. Each has an informative PNG poster.
Thirteen still diagrams are SVGs with searchable text. All data are illustrative,
...
The four small native JavaScript explorations live in interactive/math/ and
load only in Chapter 0. They cover log Taylor approximations, a scalar KKT cap,
gradient descent and Normal conjugate updating. They do not load Pyodide.
