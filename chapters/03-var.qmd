...
This standard formulation is primarily a model for conditional means and second moments. When the question is instead about the joint conditional distribution, tail co-movement, or a response that changes the whole forecast distribution, the distributional VAR extension is developed in @sec-distributional-var.
...
The stacking operation, matrix dimensions, and eigenvalue criterion used here are reviewed in @sec-matrices-transformations and @sec-eigenvalues-stability.
...
The general innovation and moving-average logic behind this representation is developed in @sec-lag-operator-foundations.
...
[Open this specification in the Interactive Lab](../interactive-lab.html?method=var).
...
The notation $\operatorname{vec}(\cdot)$ and the Kronecker-product identity are introduced in @sec-decompositions.
