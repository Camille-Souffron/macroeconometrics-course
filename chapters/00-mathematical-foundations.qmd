...
::: {.math-route}
**Reading this chapter.** Follow the first four sections in order if analysis and optimisation are unfamiliar. The examples are worked through before their general statements. The short proofs explain why the assumptions matter; the exercises at the end return to the same examples. Each animation has playback controls and a still image, and each interactive figure can be adjusted with the keyboard.
:::
...
### Combining movements and choosing coordinates
...
The inflation changes cancel. More generally, the movements available from $a$ and $b$ are their linear combinations, $\alpha a+\beta b$. Their **span** is the set of all such combinations. Here it is the whole plane: the two directions are not parallel. Had $b$ equalled $2a$, the same exercise would have generated only a line.

A family $v_1,\ldots,v_k$ is **linearly independent** when $\sum_j c_jv_j=0$ implies $c_1=\cdots=c_k=0$. Otherwise at least one direction repeats information supplied by the others. A **basis** of a space is an independent family which spans that space. Every vector then has a unique set of coordinates in that basis, and the number of basis vectors is the **dimension**.
...
The set of available movements is a **vector subspace** if it is nonempty and closed under addition and multiplication by every real scalar. These properties imply that it contains zero. A budget line such as $c_1+c_2=1$ does not contain zero and is therefore an affine set. Its *displacements*, however, satisfy $dc_1+dc_2=0$ and do form a vector subspace. This difference will be useful when we study constrained choice.

Coordinates depend on the basis. If $P=[v_1\ \cdots\ v_n]$ contains a basis of $\mathbb R^n$, the same vector can be written $x=P[x]_B$, so that $[x]_B=P^{-1}x$. The vector has not moved. We have changed the directions in which its position is measured.
...
### Matrices, rank, and recoverability {#sec-matrices-transformations}

In @eq-maths-state-system, one output coordinate combines both input coordinates. In general, a matrix $A\in\mathbb R^{m\times n}$ represents a linear map from $\mathbb R^n$ to $\mathbb R^m$:

$$
(Ax)_i=\sum_{j=1}^n a_{ij}x_j.
$$

Its columns are the images of the canonical basis vectors. If another map $B\in\mathbb R^{n\times p}$ acts first, the combined map is $AB$, with $(AB)_{ij}=\sum_k a_{ik}b_{kj}$. Thus $(m\times n)(n\times p)$ produces an $m\times p$ matrix. The order of multiplication records the order in which the maps act.

For $x\in\mathbb R^n$, $x'x$ is a scalar but $xx'$ is an $n\times n$ matrix. Transposition reverses composition: $(AB)'=B'A'$. Inversion, when possible, also reverses it: $(AB)^{-1}=B^{-1}A^{-1}$. These identities are easier to remember by following the transformations than by treating matrices as tables.
...
Observed data rarely satisfy $X\beta=y$ exactly. Let $X\in\mathbb R^{T\times k}$ and $y\in\mathbb R^T$. We seek a fitted vector in $\mathcal C(X)$ as close as possible to $y$. At the closest point $\hat y$, the residual $r=y-\hat y$ must be orthogonal to every available direction: otherwise moving a little in one of those directions would reduce the distance.
...
The projection matrix $P_X=X(X'X)^{-1}X'$ satisfies $P_X'=P_X$ and $P_X^2=P_X$. Thus $y=P_Xy+(I-P_X)y$ is a decomposition into orthogonal components. With deficient rank the fitted vector remains unique, although several coefficient vectors produce it. None of these geometric statements requires a distribution for the data.
...
For a function on $D\subseteq\mathbb R^d$, a limit at an accumulation point $a$ is the corresponding statement about *all* nearby inputs:

$$
\lim_{\substack{x\to a\\x\in D}}f(x)=\ell
\quad\Longleftrightarrow\quad
\forall\varepsilon>0\ \exists\delta>0:
\quad x\in D,\ 0<\|x-a\|_2<\delta
\Rightarrow \|f(x)-\ell\|_2<\varepsilon.
$$
...
For a $C^2$ function near $a$—one with continuous derivatives through order two—integrate twice along the segment from $a$ to $a+h$:
...
A map $F:U\subseteq\mathbb R^n\to\mathbb R^m$, with $U$ open, is **differentiable at $a$** if a linear map $J$ satisfies
...
$$
f(a+h)=f(a)+\nabla f(a)'h+\tfrac12h'H_f(a)h+o(\|h\|_2^2).
...
g_i(x)\leq0\ (i=1,\ldots,m),\qquad h_j(x)=0\ (j=1,\ldots,r),
$$
...
=f(x)+\sum_{i=1}^m\mu_i g_i(x)+\sum_{j=1}^r\lambda_j h_j(x).
$$

At a candidate $(x^*,\mu^*,\lambda^*)$, the four requirements are
...
\nabla f(x^*)+\sum_i\mu_i^*\nabla g_i(x^*)+
\sum_j\lambda_j^*\nabla h_j(x^*)=0,\\
\text{primal feasibility:}\quad&g_i(x^*)\leq0,\qquad h_j(x^*)=0,\\
\text{dual feasibility:}\quad&\mu_i^*\geq0,\\
\text{complementary slackness:}\quad&\mu_i^*g_i(x^*)=0.
\end{aligned}
$$ {#eq-kkt}
...
$$
\min_{x,y\geq0}q(x,y)=(x-1)^2+2(y-1)^2
\qquad\text{subject to}\qquad x+y\leq b,\quad b>0.
$$
...
Relaxing the cap reduces the minimum loss. With $\mathcal L=f+\mu(c-b)$, the multiplier is the *negative* derivative with respect to the right-hand side. In a utility maximisation problem expressed as minimisation of negative utility, the derivative of maximised utility has the opposite sign.
...
### OLS inference needs more than a projection {#sec-ols-assumptions}
...
With full column rank, substitution into @eq-ols-projection gives

$$
\hat\beta-\beta=(X'X)^{-1}X'u.
$$ {#eq-ols-error-decomposition}

If $E[u\mid X]=0$, the estimator is conditionally unbiased. If also $\operatorname{Var}(u\mid X)=\sigma^2I_T$, then

$$
\operatorname{Var}(\hat\beta\mid X)=\sigma^2(X'X)^{-1}.
$$ {#eq-ols-variance}

The variance assumption combines homoskedasticity and absence of conditional cross-observation covariance. Under these conditions Gauss–Markov says that any other conditionally linear unbiased estimator has covariance at least as large in the positive-semidefinite ordering. Normality is unnecessary for this comparison.

For exact small-sample tests, impose the stronger condition $u\mid X\sim N(0,\sigma^2I_T)$ and $T>k$. Then $\hat\sigma^2=\|y-X\hat\beta\|^2/(T-k)$ supplies the independent chi-square scale estimate needed for Student $t_{T-k}$ coefficient tests and $F$ tests of joint linear restrictions.

Lagged dependent variables make $E[u\mid X]=0$ particularly demanding. A current innovation can be uncorrelated with its own past regressors while entering future rows of the full matrix $X$. Finite-sample conditional unbiasedness need not follow. Time-series consistency instead commonly rests on $T^{-1}X'X\xrightarrow{p}Q\succ0$ and $T^{-1}X'u\xrightarrow{p}0$. A CLT for $T^{-1/2}X'u$ then supplies asymptotic inference.

With independent heteroskedastic observations and suitable regularity, the HC0 estimator of $\operatorname{Var}(\hat\beta)$ is

$$
(X'X)^{-1}\left(\sum_{t=1}^T x_tx_t'\hat u_t^2\right)(X'X)^{-1}.
$$

This expression is on the scale of $\hat\beta$, not $\sqrt T(\hat\beta-\beta)$. HAC estimators add weighted sample lag covariances of $x_t\hat u_t$ to allow serial correlation; cluster estimators allow dependence within groups under assumptions on the number and dependence of clusters. These adjustments do not repair endogeneity.
...
For linear algebra and projection geometry, see Strang [@Strang2016]. Lebl's open textbook [*Basic Analysis*, volumes I and II](https://www.jirka.org/ra/) develops real and multivariable analysis, including the inverse and implicit function theorems. Boyd and Vandenberghe's [*Convex Optimization*, chapters 2–5](https://web.stanford.edu/~boyd/cvxbook/bv_cvxbook.pdf) covers convexity, duality, Slater's condition and KKT sufficiency. For nonconvex first- and second-order conditions, see Nocedal and Wright [@NocedalWright2006], chapter 12. Casella and Berger [@CasellaBerger2002] cover probability and inference; van der Vaart [@vanDerVaart1998] develops asymptotic arguments, and Hamilton [@Hamilton1994] connects them to time series.
