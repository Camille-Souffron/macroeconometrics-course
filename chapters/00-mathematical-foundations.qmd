...
Fourier analysis represents a finite data vector in an orthogonal basis of oscillating functions. A sinusoid $\cos(\omega t+\varphi)$ has angular frequency $\omega$, period $2\pi/\omega$ when $\omega>0$, and phase $\varphi$. Euler's identity $e^{i\omega t}=\cos(\omega t)+i\sin(\omega t)$ packages the cosine and sine coordinates into one complex number.
